...
const server = new McpServer({ name: 'gdex-mcp-server', version: '4.5.0' }, { capabilities: { tools: {} } });
const skills = await loadSkills();
// --- Tool: search_gdex_docs ---
server.tool('search_gdex_docs', 'Search GDEX documentation and skill files by keyword. Returns matching skill sections.', { query: z.string().describe('Search query — keywords to find across all GDEX skills and documentation') }, async ({ query }) => {
...
server.tool('get_sdk_pattern', 'Get TypeScript code patterns for common GDEX SDK operations. Returns working code examples.', { operation: z.enum(SDK_OPERATIONS).describe('The SDK operation to get code patterns for') }, async ({ operation }) => {
    const skillMap = {
...
// --- Tool: get_api_info ---
server.tool('get_api_info', 'Get GDEX API endpoint details — URL, method, parameters, and response format.', { endpoint: z.string().describe('Endpoint name or keyword (e.g., "buy_token", "hl_create_order", "deposit", "portfolio")') }, async ({ endpoint }) => {
...
server.tool('explain_workflow', 'Explain an end-to-end GDEX trading workflow with step-by-step instructions and code.', { workflow: z.enum(WORKFLOWS).describe('The workflow to explain') }, async ({ workflow }) => {
...
// --- Tool: get_chain_info ---
server.tool('get_chain_info', 'Get supported chains, chain IDs, DEXes, and capabilities.', { chain: z.string().optional().describe('Optional: filter by chain name (e.g., "solana", "base", "arbitrum"). Omit for all chains.') }, async ({ chain }) => {
...
- HIP-3 perp DEXes: query \`/v1/hl/perp_dexes\` and pass \`dex\` parameter on read endpoints; use \`_all\` variants (clearinghouse_state_all, open_orders_all) for combined views`;
    if (chain) {
        ...
        const lines = chainTable.split('\n').filter(line => line.toLowerCase().includes(chainLower) || line.startsWith('|') && line.includes('---') || line.startsWith('#') || line.startsWith('**'));
        return { content: [{ type: 'text', text: lines.join('\n') }] };
    }
    return { content: [{ type: 'text', text: chainTable }] };
});
// --- Tool: get_trading_guide ---
server.tool('get_trading_guide', 'Get a complete trading guide for spot, perp, or limit order trading.', { type: z.enum(['spot', 'perp', 'limit']).describe('Trading type: spot, perp, or limit') }, async ({ type }) => {
    const skillMap = {
...
server.tool('get_copy_trade_guide', 'Get a copy trading guide for Solana spot or HyperLiquid perp copy trading.', { chain: z.enum(['solana', 'hyperliquid']).describe('Platform: solana (spot copy) or hyperliquid (perp copy)') }, async ({ chain }) => {
...
server.tool('get_component_guide', 'Get React UI component patterns for GDEX trading interfaces.', { component: z.string().describe('Component name or category (e.g., "SpotTradeForm", "PositionTable", "portfolio", "theming", "wallet", "page-layouts")') }, async ({ component }) => {
